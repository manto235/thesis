...
Le \textit{crawler} fonctionne de la manière suivante : tout d'abord, il vérifie que le répertoire donné en argument est accessible en écriture et il crée le fichier de logs. Ensuite, il charge la liste des sites à visiter depuis le fichier spécifié lors du lancement du programme. Puis, le profil choisi est chargé et différents paramètres de configurations sont également réglés avant que le pilote ne soit initialisé et qu'il lance Firefox. Après le lancement du pilote, un temps d'attente de 5 secondes laisse le temps à Firefox de se lancer et de charger ses extensions.
